...
const { createErrorMessages } = authentications;
...
const orderContents = {
  ...
  quantity: Joi.number().min(1).positive().required().messages(
    createErrorMessages(
      'number',
...
const placeOrder = (data) => {
  const schema = Joi.object({
    ...
    contents: 
    Joi.array().items(orderContents).min(1).ordered(orderContents).required().messages(createErrorMessages(
      'array',
      `${messages.orderEmpty}`,
      `${messages.orderEmpty}`,
      null,
